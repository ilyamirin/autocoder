...
DEFAULT_TASKS = [
    ...
    task(
        id="BL-013",
        title="Минимально обогатить данные возвратов без изменения расчётов",
        kind="quality",
        status="backlog",
        summary=(
            "Нужно сделать аналитику и таблицы менее игрушечными, но не трогать бизнес-логику. "
            "Ожидается минимальный патч в seed-данных и тестах, без правок расчётов и шаблонов."
        ),
        acceptance_criteria=(
            "1. В тестовом наборе заказов появляется как минимум ещё один returned и ещё один cancelled кейс.\n"
            "2. Новых записей ровно столько, сколько нужно для задачи; лишние delivered или новые сущности не добавляются.\n"
            "3. Расчёты в domain.py и шаблоны страниц не меняются.\n"
            "4. Изменены только данные и связанные с ними тесты в пределах разрешённого профиля.\n"
            "5. После изменения зелёными остаются локальные тесты и Gitea Actions."
        ),
        target_area="data",
        execution_risk="review",
        visual_score=4,
        realism_score=5,
        agent_score=3,
    ),
    task(
        id="BL-014",
        title="Исправить долю возвратов только локальной доменной правкой",
        kind="bug",
        status="backlog",
        summary=(
            "Return rate на дашборде должен считаться строго по fulfilled-заказам, но решение должно "
            "оставаться локальным: без правок UI, seed-набора и без лишнего рефакторинга."
        ),
        acceptance_criteria=(
            "1. Return rate считает returned в числителе и delivered + returned в знаменателе.\n"
            "2. Processing и cancelled не входят в знаменатель.\n"
            "3. Данные seed-набора и HTML-шаблоны не меняются.\n"
            "4. Для бага добавлена или обновлена ровно необходимая регрессионная проверка.\n"
            "5. Патч ограничен domain.py и тестами в рамках профиля, без побочных улучшений."
        ),
        target_area="finance",
        execution_risk="medium",
        visual_score=4,
    ...
    task(
        id="BL-015",
        title="Показать low stock без новых компонентов и API",
        kind="feature",
        status="backlog",
        summary=(
            "На странице Products нужно явно показать low stock, но без новых страниц, новых маршрутов "
            "и без расширения модели данных. Это проверка на минимальное UI-изменение в пределах текущей таблицы."
        ),
        acceptance_criteria=(
            "1. На странице Products low stock заметен прямо в существующей таблице товаров.\n"
            "2. Решение использует текущие данные товара и существующий шаблон страницы.\n"
            "3. Новые роуты, новые API и новые сущности не добавляются.\n"
            "4. Изменения ограничены локальной domain/app/template правкой в рамках профиля.\n"
            "5. Для поведения есть тест или smoke-проверка рендера страницы."
        ),
        target_area="products",
        execution_risk="medium",
        visual_score=5,
    ...
    task(
        id="BL-016",
        title="Добавить live build badge минимальным platform-патчем",
        kind="feature",
        status="backlog",
        summary=(
            "Нужно вывести в live pet-app id последней задеплоенной задачи или live commit, но не расползтись "
            "по нескольким сервисам и не переизобретать текущий контур деплоя."
        ),
        acceptance_criteria=(
            "1. В live pet-app появляется компактный build badge с task id или live commit текущего деплоя.\n"
            "2. Значение берётся из уже существующих runtime-данных, без новой внешней интеграции.\n"
            "3. Патч не меняет Kanboard, Gitea и CI-конфигурацию, если это не требуется напрямую для вывода бейджа.\n"
            "4. Изменение остаётся локальным для platform-профиля и не затрагивает больше сервисов, чем нужно.\n"
            "5. После следующего успешного прогона агента значение бейджа обновляется автоматически."
        ),
        target_area="platform",
        execution_risk="review",
        visual_score=5,
